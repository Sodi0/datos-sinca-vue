    ...
    <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-16 items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <RouterLink to="/" class="flex items-center gap-2" title="Ir al inicio">
            <img src="../assets/logo.png" alt="Logo" class="h-12 w-auto md:h-14 lg:h-14 object-contain"
              loading="eager" />
          </RouterLink>
        </div>
        ...
        <nav class="hidden md:flex gap-8 text-sm font-medium">
          <RouterLink to="/" class="text-gray-700 hover:text-blue-600 transition" title="Ir al inicio">
            Inicio
          </RouterLink>
        ...
        <!-- Menú móvil -->
        <div class="md:hidden">
          <input type="checkbox" id="mobile-menu-toggle" class="hidden peer" v-model="menuOpen" />

          <label for="mobile-menu-toggle" class="p-2 text-gray-700 hover:text-blue-600 cursor-pointer block"
            aria-label="Abrir menú de navegación" :aria-expanded="menuOpen">
            <div class="relative w-6 h-6">
              <span class="absolute top-1 left-0 w-6 h-0.5 bg-current transition-all duration-300"
                :class="{ 'rotate-45 top-2.5': menuOpen }"></span>
          ...
          <transition name="slide-down">
            <div v-if="menuOpen" class="absolute left-0 right-0 top-16 bg-white shadow-lg border-t">
              <nav class="px-4 py-4 space-y-2">
                <RouterLink to="/"
                  class="block py-3 px-2 text-sm text-gray-700 hover:text-blue-600 hover:bg-gray-50 rounded transition"
                  title="Ir al inicio" @click="menuOpen = false">
                  Inicio
                </RouterLink>
                <RouterLink to="/mapa"
                  class="block py-3 px-2 text-sm text-gray-700 hover:text-blue-600 hover:bg-gray-50 rounded transition"
